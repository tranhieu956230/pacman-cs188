        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                if (nextx, nexty) in state[1]:
                    newTuple = tuple(x for x in state[1]
                                     if x != (nextx, nexty))
                    successors.append((((nextx, nexty), newTuple), action, 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner in state[1]:
            md = util.manhattanDistance(corner, state[0])
            if md > heuristic:
                heuristic = md
...
def foodHeuristic(state, problem):
    ...
    for x, y in foodGrid.asList():
        if foodGrid[x][y]:
            md = mazeDistance((x, y), position, problem.startingGameState)
            if md > heuristic:
                heuristic = md
    return heuristic
